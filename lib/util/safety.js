...
// This will only ever be run in strict mode.
function verify(opts) {
  opts = opts || {};
  
  if (process.geteuid == null) {
    // This was run as sudo, but we can't do anything about it!
    log.fatal("process.geteuid is not defined, can't verify userid, aborting!");
  }
  ...
  let currentuid = parseInt(process.geteuid());
  if (currentuid !== uid) {
    log.fatal(`Aborting due to userid mismatch: Expected ${uid}, got ${currentuid}`);
  }
  log.trace(`Running as correct uid! (${uid})`);
}
...
module.exports = {
  ...
  deEscalate(opts) {
    ...
    if (process.env.SUDO_UID == null) {
      ...
      if (strict) {
        log.trace("Not running as sudo, verifying correct uid...");
        verify(opts);
      } else {
        log.trace("Not running as sudo, nothing to do!");
      }
      return;
    }
